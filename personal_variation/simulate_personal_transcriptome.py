...
def main():
    ...
    if len(sys.argv) < 6:
        sys.stderr.write('Usage: %s <transcripts.gtf> <transcripts.fa> <outname_tag> <phased_germline_var.vcf-pattern> <somatic_va.vcf-pattern>\n' % sys.argv[0])
        sys.exit(1)

    fname_gtf = sys.argv[1]
    fname_fa = sys.argv[2]
    outtag = sys.argv[3]
    germline_vcf_pattern = sys.argv[4]
    somatic_vcf_pattern = sys.argv[5]
    ...
    personalize_transcriptome(fname_fa, outtag, transcripts, variants_germline, variants_somatic) 

    write_variant_positions(fname_fa, outtag, variants_germline, variants_somatic)
...
def personalize_transcript(header, seq, transcripts, variants_germ, variants_som):
    ...
    return seq_ht1_g, seq_ht2_g, seq_ht1_gs, seq_ht2_gs, var_cnt1_g, var_cnt2_g, var_cnt1_gs, var_cnt2_gs
...
def write_variant_positions(fname_fa, outtag, variants_germline, variants_somatic):
    ...
    with open(re.sub(r'.fa$', '', fname_fa) + '.' + outtag + '.germline.vcf', 'w') as fh:
        fh.write(info + '\n')
    ...
    pickle.dump(variants_germline, open(re.sub(r'.fa$', '', fname_fa) + '.' + outtag + '.germline.pickle', 'wb'))

    ### write somatic in vcf
    with open(re.sub(r'.fa$', '', fname_fa) + '.' + outtag + '.somatic.vcf', 'w') as fh:
        fh.write(info + '\n')
        for chrm in variants_somatic:
            fh.write('##contig=<ID=%s>\n' % chrm)
        fh.write(header + '\n')
    ...
    pickle.dump(variants_somatic, open(re.sub(r'.fa$', '', fname_fa) + '.' + outtag + '.somatic.pickle', 'wb'))


def personalize_transcriptome(fname_fa, outtag, transcripts, variants_germline, variants_somatic):
    
    header = None
    seq = []
    fh_ht1 = open(re.sub(r'.fa$', '', fname_fa) + '.' + outtag + '.ht1.fa', 'w')
    fh_ht2 = open(re.sub(r'.fa$', '', fname_fa) + '.' + outtag + '.ht2.fa', 'w')
    fh_ht1_som = open(re.sub(r'.fa$', '', fname_fa) + '.' + outtag + '.ht1.som.fa', 'w')
    fh_ht2_som = open(re.sub(r'.fa$', '', fname_fa) + '.' + outtag + '.ht2.som.fa', 'w')
    ...
    for line in open(fname_fa, 'r'):
        if line.startswith('>'):
            if len(seq) > 0:
                seq_ht1_g, seq_ht2_g, seq_ht1_gs, seq_ht2_gs, cnt1_g, cnt2_g, cnt1_gs, cnt2_gs = personalize_transcript(header, ''.join(seq), transcripts, variants_germline, variants_somatic)
                var_cnt1_g += cnt1_g
                var_cnt2_g += cnt2_g
                var_cnt1_gs += cnt1_gs
                var_cnt2_gs += cnt2_gs
...
def load_variants_cosmic(fname_vcf_pattern, tx_intervals, variants_germline, loh_frac=0.25):
